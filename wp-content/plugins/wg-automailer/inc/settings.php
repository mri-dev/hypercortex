...
class WGRSSMailer_Settings
{
  ...
  public function saving_posted_add_automation()
  {
    ...
    if ( !$errors->errors && !wp_verify_nonce( $_POST['wgrss-settings-adder'], 'wgrss-settings-adder' ) ) {
      $errors->add('nopriv', 'Hibás azonosítás és hozzáférés.');
    } else {
      ...
      if ($is_editing)
      {
        ...
        $posttypes = (!isset($_POST['post_types'])) ? NULL: implode($_POST['post_types'],',');
  ...
  public function collectAvaiableTerms()
  {
    $taxs = array('category');
    ...
    foreach ((array)$post_types as $key => $value) {
      $taxi = get_object_taxonomies( $value );
      foreach ((array)$taxi as $t) {
        $taxis[$t] = $value;
        $taxs[] = $t;
      }
    }
    ...
    foreach ( (array)$terms as $term ) {
      $term->pt = get_post_type_object($taxis[$term->taxonomy]);
      $xc = ($term->pt) ? $term->pt->name : 'category';

      $xterm[$xc]['name'] =  ($term->pt) ? $term->pt->label : 'Bejegyzés';
      $xterm[$xc]['items'][] = $term;
    }
  ...
  public function collectPostTypes()
  {
    $pts = array();
    $pts[] = (object)array(
      'name' => 'post',
    ...
    foreach ((array)$post_types as $p) {
      $pts[] = get_post_type_object($p);
    }
  ...
  public function wgrss_settings_page_adder()
  {
    ...
    $categories = $this->collectAvaiableTerms();
    $posttypes = $this->collectPostTypes();
